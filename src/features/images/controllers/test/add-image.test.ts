...
describe('Add', () => {
  ...
  describe('profileImage', () => {
    it('should call image upload method', async () => {
      ...
      expect(cloudinaryUploads.upload).toHaveBeenCalledWith(req.body.image, req.currentUser?.userId, true, true);
    ...
    it('should call updateSingleUserItemInCache method', async () => {
      ...
      const url = 'https://res.cloudinary.com/fastrobot/image/upload/v1234/123456';
    ...
    it('should call addImageJob method', async () => {
      ...
      expect(imageQueue.addImageJob).toHaveBeenCalledWith('addUserProfileImageToDB', {
        ...
        value: 'https://res.cloudinary.com/fastrobot/image/upload/v1234/123456',
        imgId: '123456',
        imgVersion: '1234'
      });
      expect(res.status).toHaveBeenCalledWith(200);
  ...
  describe('backgroundImage', () => {
    it('should upload new image', async () => {
      ...
      const res: Response = imagesMockResponse();
      jest.spyOn(cloudinaryUploads, 'upload').mockImplementation((): any => Promise.resolve({ version: '2467', public_id: '987654' }));

      await Add.prototype.backgroundImage(req, res);
      expect(cloudinaryUploads.upload).toHaveBeenCalledWith(req.body.image);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
        message: 'Image added successfully'
      });
    ...
    it('should not upload existing image', async () => {
      const req: Request = imagesMockRequest(
        {},
        { image: 'https://res.cloudinary.com/fastrobot/image/upload/v1234/123456' },
        authUserPayload
      ) as Request;
      const res: Response = imagesMockResponse();
      jest.spyOn(cloudinaryUploads, 'upload');

      await Add.prototype.backgroundImage(req, res);
      expect(cloudinaryUploads.upload).not.toHaveBeenCalledWith(req.body.image);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
        message: 'Image added successfully'
      });
