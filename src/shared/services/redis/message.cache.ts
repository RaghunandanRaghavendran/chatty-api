...
export class MessageCache extends BaseCache {
  ...
  public async getUserConversationList(key: string): Promise<IMessageData[]> {
    try {
      ...
      for (const item of userChatList) {
        const chatItem: IChatList = ExtensionMetod.parseJson(item) as IChatList;
        const lastMessage: string = (await this.client.LINDEX(`messages:${chatItem.conversationId}`, -1)) as string;
        conversationChatList.push(ExtensionMetod.parseJson(lastMessage));
      }
  ...
  public async getChatMessagesFromCache(senderId: string, receiverId: string): Promise<IMessageData[]> {
    try {
      ...
      const parsedReceiver: IChatList = ExtensionMetod.parseJson(receiver) as IChatList;
      if (parsedReceiver) {
        ...
        for (const item of userMessages) {
          const chatItem = ExtensionMetod.parseJson(item) as IMessageData;
  ...
  public async markMessageAsDeleted(senderId: string, receiverId: string, messageId: string, type: string): Promise<IMessageData> {
    try {
      ...
      const chatItem = ExtensionMetod.parseJson(message) as IMessageData;
      if (type === 'deleteForMe') {
      ...
      return ExtensionMetod.parseJson(lastMessage) as IMessageData;
    } catch (error) {
      log.error(error);
      throw new ServerError('Server error. Try again.');
    }
  ...
  public async updateChatMessages(senderId: string, receiverId: string): Promise<IMessageData> {
    try {
      ...
      const unreadMessages: string[] = filter(messages, (listItem: string) => !ExtensionMetod.parseJson(listItem).isRead);
  ...
  public async updateMessageReaction(
    ...
    try {
      ...
      const parsedMessage: IMessageData = ExtensionMetod.parseJson(message) as IMessageData;
      ...
      return ExtensionMetod.parseJson(updatedMessage) as IMessageData;
    } catch (error) {
      log.error(error);
      throw new ServerError('Server error. Try again.');
    }
  ...
  private async getChatUsersList(): Promise<IChatUsers[]> {
    ...
    for (const item of chatUsers) {
      const chatUser: IChatUsers = ExtensionMetod.parseJson(item) as IChatUsers;
