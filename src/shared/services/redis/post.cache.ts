...
export class PostCache extends BaseCache {
  ...
  public async getPostsFromCache(key: string, start: number, end: number): Promise<IPostDocument[]> {
    try {
      ...
      for (const post of replies as IPostDocument[]) {
        post.commentsCount = ExtensionMetod.parseJson(`${post.commentsCount}`) as number;
        post.reactions = ExtensionMetod.parseJson(`${post.reactions}`) as IReactions;
        post.createdAt = new Date(ExtensionMetod.parseJson(`${post.createdAt}`)) as Date;
  ...
  public async updatePostInCache(key: string, updatedPost: IPostDocument): Promise<IPostDocument> {
    ...
    try {
      ...
      postReply[0].commentsCount = ExtensionMetod.parseJson(`${postReply[0].commentsCount}`) as number;
      postReply[0].reactions = ExtensionMetod.parseJson(`${postReply[0].reactions}`) as IReactions;
      postReply[0].createdAt = new Date(ExtensionMetod.parseJson(`${postReply[0].createdAt}`)) as Date;
